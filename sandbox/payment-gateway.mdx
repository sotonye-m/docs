...
```bash
curl -X POST 'https://crypto-sandbox.yativo.com/api/crypto-gateway/payments' \
...
```bash
curl -X POST 'https://crypto-sandbox.yativo.com/api/sandbox/simulate-gateway-payment' \
  -H 'Authorization: Bearer YOUR_ACCESS_TOKEN' \
  -H 'Content-Type: application/json' \
  -d '{
