---
title: "Test: IBAN"
description: "Test the IBAN onboarding flow in the Yativo Crypto sandbox"
---

## Overview

Use the sandbox to test the complete IBAN onboarding flow. The sandbox runs on a **testnet** — no real IBAN is provisioned, and no real bank transfers can be sent to sandbox IBANs.

**Sandbox base URL:** `https://crypto-sandbox.yativo.com/api/v1/`

<Note>
  In the sandbox, IBAN operations run on a testnet. No real IBANs are assigned and no real bank transfers can be received. Use the sandbox to verify your integration logic and status-handling code.
</Note>

---

## Step 1 — Get IBAN Options

Retrieve available options and requirements for standalone IBAN onboarding.

```
GET https://crypto-sandbox.yativo.com/api/v1/iban/options
```

```bash cURL
curl -X GET 'https://crypto-sandbox.yativo.com/api/v1/iban/options' \
  -H 'Authorization: Bearer YOUR_SANDBOX_TOKEN'
```

```json Response
{
  "status": "success",
  "data": {
    "supported_currencies": ["EUR"],
    "supported_networks": ["SEPA", "SEPA_INSTANT"],
    "kyc_required": true,
    "supported_payout_tokens": ["USDC", "USDT"],
    "supported_payout_chains": ["ethereum", "polygon", "solana"],
    "sandbox_note": "IBAN options in sandbox mirror production. Uses testnet. No real IBANs provisioned."
  }
}
```

---

## Step 2 — Initialize Onboarding

```
POST https://crypto-sandbox.yativo.com/api/v1/iban/onboarding/init
```

<ParamField body="email" type="string" required>
  The applicant's email address.
</ParamField>

<ParamField body="customer_id" type="string">
  Your internal customer identifier. Links the IBAN to an existing customer record.
</ParamField>

```bash cURL
curl -X POST 'https://crypto-sandbox.yativo.com/api/v1/iban/onboarding/init' \
  -H 'Authorization: Bearer YOUR_SANDBOX_TOKEN' \
  -H 'Content-Type: application/json' \
  -d '{
    "email": "sandbox-iban-test@example.com",
    "customer_id": "cus_sandbox_iban_001"
  }'
```

```json Response
{
  "status": "success",
  "message": "IBAN onboarding initialized",
  "data": {
    "onboarding_id": "onb_01HX9KZMB3F7VNQP8R2WDGTAAA",
    "customer_id": "cus_sandbox_iban_001",
    "email": "sandbox-iban-test@example.com",
    "status": "pending_kyc",
    "next_step": "kyc",
    "created_at": "2026-03-28T21:00:00Z"
  }
}
```

---

## Step 3 — Submit KYC

```
POST https://crypto-sandbox.yativo.com/api/v1/iban/onboarding/kyc
```

<ParamField body="customer_id" type="string" required>
  Your internal customer identifier.
</ParamField>

<ParamField body="kyc" type="object" required>
  KYC data for the applicant.
</ParamField>

```bash cURL
curl -X POST 'https://crypto-sandbox.yativo.com/api/v1/iban/onboarding/kyc' \
  -H 'Authorization: Bearer YOUR_SANDBOX_TOKEN' \
  -H 'Content-Type: application/json' \
  -d '{
    "customer_id": "cus_sandbox_iban_001",
    "kyc": {
      "method": "direct_docs",
      "first_name": "Sandbox",
      "last_name": "User",
      "date_of_birth": "1990-01-01",
      "nationality": "DE"
    }
  }'
```

```json Response
{
  "status": "success",
  "data": {
    "customer_id": "cus_sandbox_iban_001",
    "kyc_status": "pending",
    "submitted_at": "2026-03-28T21:05:00Z"
  }
}
```

---

## Step 4 — Request IBAN

Once KYC is approved, request IBAN provisioning.

```
POST https://crypto-sandbox.yativo.com/api/v1/iban/iban/request
```

<ParamField body="customer_id" type="string" required>
  Your internal customer identifier.
</ParamField>

<ParamField body="payout_token" type="string" required>
  Token to receive converted funds in: `"USDC"`.
</ParamField>

<ParamField body="payout_chain" type="string" required>
  Chain for payouts: `"solana"`, `"ethereum"`, `"polygon"`.
</ParamField>

<ParamField body="payout_address" type="string" required>
  Testnet wallet address for payouts.
</ParamField>

```bash cURL
curl -X POST 'https://crypto-sandbox.yativo.com/api/v1/iban/iban/request' \
  -H 'Authorization: Bearer YOUR_SANDBOX_TOKEN' \
  -H 'Content-Type: application/json' \
  -d '{
    "customer_id": "cus_sandbox_iban_001",
    "payout_token": "USDC",
    "payout_chain": "solana",
    "payout_address": "GY1EZGdpiJNyx2BSKq8rfTDRe5K8Bb6Cf2Bn1pdmE2o1"
  }'
```

<ResponseExample>
```json Success
{
  "status": "success",
  "data": {
    "customer_id": "cus_sandbox_iban_001",
    "iban_request_id": "ibanreq_01HX9KZMB3F7VNQP8R2WDGTBBB",
    "status": "provisioning"
  }
}
```

```json KYC Not Yet Approved
{
  "status": "error",
  "message": "KYC must be approved before requesting an IBAN.",
  "error_code": "KYC_NOT_APPROVED"
}
```
</ResponseExample>

---

## Step 5 — Check Status

Poll the status endpoint to track the flow.

```
GET https://crypto-sandbox.yativo.com/api/v1/iban/status
```

<ParamField query="customer_id" type="string" required>
  Your internal customer identifier.
</ParamField>

```bash cURL
curl -X GET 'https://crypto-sandbox.yativo.com/api/v1/iban/status?customer_id=cus_sandbox_iban_001' \
  -H 'Authorization: Bearer YOUR_SANDBOX_TOKEN'
```

<ResponseExample>
```json IBAN Active (Sandbox)
{
  "status": "success",
  "data": {
    "customer_id": "cus_sandbox_iban_001",
    "overall_status": "active",
    "kyc_status": "approved",
    "iban_status": "active",
    "iban": "DE89370400440532013099",
    "bic": "SSKMDEMMXXX",
    "account_holder_name": "Sandbox User",
    "currency": "EUR",
    "payout_token": "USDC",
    "payout_chain": "solana",
    "payout_address": "GY1EZGdpiJNyx2BSKq8rfTDRe5K8Bb6Cf2Bn1pdmE2o1",
    "activated_at": "2026-03-28T21:07:00Z",
    "sandbox_note": "Testnet IBAN. Cannot receive real bank transfers."
  }
}
```

```json Pending KYC
{
  "status": "success",
  "data": {
    "customer_id": "cus_sandbox_iban_001",
    "overall_status": "pending_kyc",
    "kyc_status": "pending",
    "iban_status": "not_requested"
  }
}
```
</ResponseExample>

---

## Sandbox Limitations

- IBANs run on a **testnet** — no real banking connectivity
- Cannot receive transfers from real banks — testnet only
- The IBAN number returned is a testnet IBAN with no real banking connectivity

---

## End-to-End Test Script

```bash Shell script
#!/bin/bash

BASE_URL="https://crypto-sandbox.yativo.com/api"
TOKEN="YOUR_SANDBOX_TOKEN"
CUSTOMER_ID="sandbox_iban_$(date +%s)"

echo "=== Step 1: Init onboarding ==="
curl -s -X POST "$BASE_URL/iban/onboarding/init" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d "{\"email\":\"test+$CUSTOMER_ID@example.com\",\"customer_id\":\"$CUSTOMER_ID\"}" | jq .

echo "=== Step 2: Submit KYC ==="
curl -s -X POST "$BASE_URL/iban/onboarding/kyc" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d "{\"customer_id\":\"$CUSTOMER_ID\",\"kyc\":{\"method\":\"direct_docs\",\"first_name\":\"Sandbox\",\"last_name\":\"User\",\"date_of_birth\":\"1990-01-01\",\"nationality\":\"DE\"}}" | jq .

echo "=== Step 3: Check status ==="
curl -s -X GET "$BASE_URL/iban/status?customer_id=$CUSTOMER_ID" \
  -H "Authorization: Bearer $TOKEN" | jq .

echo "=== Step 4: Request IBAN ==="
curl -s -X POST "$BASE_URL/iban/iban/request" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d "{\"customer_id\":\"$CUSTOMER_ID\",\"payout_token\":\"USDC\",\"payout_chain\":\"solana\",\"payout_address\":\"GY1EZGdpiJNyx2BSKq8rfTDRe5K8Bb6Cf2Bn1pdmE2o1\"}" | jq .
```

---

## Testing Error Scenarios

<Accordion title="Test IBAN request before KYC approval">
  Submit a KYC request and immediately call the IBAN request endpoint before KYC is approved. You should receive a `KYC_NOT_APPROVED` error.
</Accordion>

<Accordion title="Test duplicate customer ID">
  Submit two init calls with the same `customer_id` to test conflict handling in your application.
</Accordion>

<Tip>
  Use unique `customer_id` values for each test run (e.g., append a timestamp) to avoid state conflicts between test runs.
</Tip>
