...
```bash Dark theme (white card digits)
curl -X POST 'https://crypto-api.yativo.com/api/v1/yativo-card/yc_01HX9KZMB3F7VNQP8R2WDGT4E5/cards/card_01HX9KZMB3F7VNQP8R2WDGT4E5/view-token' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
...
When you pass dark colors in `theme.panel_color` or `theme.background_color`, the page wrapper automatically detects the luminance of those colors and applies `color-scheme: dark` to the card data frame. The card SDK picks this up and renders the card digits in white — no extra configuration in your request is required.

**One-time setup:** dark-mode digit rendering must be activated at the program level before dark themes take effect. Contact Yativo to enable it for your program. After activation, any request where `panel_color` or `background_color` is dark will automatically render white digits.

```bash
# Dark panel → white digits (once program-level activation is complete)
curl -X POST '.../view-token' \
  -d '{
    "enabled_views": ["data"],
    "theme": {
...
<Warning>
  **Dark-theme digit text requires program-level setup.** Dark-mode digit rendering must be enabled for your program by Yativo before dark `panel_color`/`background_color` values produce white digits. Once activated, passing any dark color in those fields is sufficient — no per-request flag is needed.
</Warning>
