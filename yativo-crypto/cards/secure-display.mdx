...
```json Success
{
  "status": "success",
  "data": {
    "secure_view_url": "https://crypto-api.yativo.com/api/v1/card-view?token=eyJhbGciOiJIUzI1NiIs...",
...
interface ViewTokenResponse {
  ...
  pin_set: boolean;                 // Whether the cardholder has set a PIN — ["data"] is blocked until true
...
| `["pin"]` | PIN setup / change form — no card number tab |
| `["data", "pin"]` | Both tabs, user can switch between them |

<Note>
  The view-token response includes `pin_set` — the last known state from the `physical.card.pin.changed` webhook. Use it to decide in your UI whether to prompt the cardholder to set or change their PIN before viewing card details. `pin_set: false` does not block the token request; it is informational only.
</Note>
