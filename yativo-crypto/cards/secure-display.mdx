...
**Layer 2 — Card data area** (extracted and applied as SDK inline styles)

The card number, expiry, and CVV are rendered inside a cross-origin secure iframe. Yativo extracts a specific set of selectors from your stylesheet and forwards them as inline style overrides to the secure card SDK. Only these selectors reach inside the iframe.
...
| `.pse-card-value` | Field values — the actual card number, expiry, CVV text |
...
.pse-card-value { color: #111827; font-size: 14px; font-family: Inter, sans-serif; }
...
<Warning>
  **Keep the page shell light.** The card data iframe is rendered by the card network in light mode and cannot be changed. Applying a dark background to `.frame` or `body` will make the card number, expiry, and CVV unreadable. Use light backgrounds for the page shell and style the card data area with dark text on white.
</Warning>
