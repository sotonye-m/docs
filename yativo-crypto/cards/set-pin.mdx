...
<Steps>
  ...
  <Step title="Card network confirms via webhook">
...
`pin_set` is updated when Yativo receives the `physical.card.pin.changed` webhook from the card network. It is **not** sourced from a direct API lookup — treat it as the last known state.
...
| `pin_set` source | Webhook only — not from a direct card network API lookup |
