...
|---------|-----------|----------|
| USDC | USDC bridge from SOL/XDC | Funding cards in USD regions |
| EURe | EURe bridge or swap | Funding cards in EUR regions |
| GBPe | GBPe bridge or swap | Funding cards in GBP regions |

<Note>
  The underlying on-chain address is abstracted — balances are presented as `usdc`, `eure`, and `gbpe`. To move from USDC balance to EURe balance, use the swap endpoint below.
</Note>
...
Swap USDC ↔ EURe ↔ GBPe within your settlement wallet.
...
Transfer from your master wallet to a customer's card wallet. The destination currency is automatically detected from the customer's card region (e.g. UK customers receive GBPe, EU customers receive EURe).
...
<Note>
  The token mismatch check is enforced at the API level. For example, if a customer's card is in a EUR region (EURe wallet), you cannot fund them with a USDC destination — the API returns `TOKEN_MISMATCH` and tells you the required token. Swap your USDC balance to EURe first, then fund.
</Note>
