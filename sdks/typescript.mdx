...
The widget requires a short-lived **ephemeral token** generated server-side — never pass your `apiSecret` to client code.

**Step 1 — Generate an ephemeral token on your server:**

```typescript
// Server-side only (Node.js / backend route)
const ephemeral = await sdk.cards.getEphemeralToken({
  yativoCardId: "yc_abc123",
  cardId:       "card_mno345",
});
// Send ephemeral.token to your frontend — it expires in ~5 minutes
```

**Step 2 — Mount the widget with the ephemeral token:**

```typescript
import { YativoCardEmbed } from "@yativo/crypto-sdk";

const embed = YativoCardEmbed.mount("#card-container", {
  ephemeralToken: ephemeralTokenFromServer,   // from your backend
  theme: {
    background: "#1a1a2e",
    textColor:  "#ffffff",
...
<Warning>
  Never pass `apiKey` or `apiSecret` to the card widget in client-side code. Use the ephemeral token pattern above. See [Get Ephemeral Token](/api-reference/cards/ephemeral-token).
</Warning>
