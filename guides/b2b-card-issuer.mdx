...
<Tip>
  When KYC transitions to `approved`, two things happen automatically:

  1. **Terms are accepted** on behalf of the customer. You will see `"terms_accepted": true` in this response. There is no customer-side `accept-terms` endpoint — this happens silently. (The `POST /yativo-card/{id}/terms` endpoint only exists for own-account users, not for B2B customers.)

  2. **The card wallet (Safe) is deployed in the background.** You do not need to call a wallet deployment endpoint. Deployment typically completes within 60 seconds.

  If card creation later fails with a Safe-not-ready error, use the fallback endpoint to force-trigger or verify deployment — it is idempotent and safe to call even if already deployed:

  ```bash
  POST /v1/yativo-card/customers/{yativoCardId}/safe/deploy
  ```

  A `200` response with `"already_deployed": true` confirms the Safe is ready.
</Tip>
...
| Looking for a customer-side `accept-terms` endpoint | No such endpoint exists in the customer namespace | Terms are auto-accepted when KYC is approved. Check `terms_accepted: true` in the KYC status response. |
...
| **Deploy Safe (fallback)** | `POST` | `/v1/yativo-card/customers/{id}/safe/deploy` |
