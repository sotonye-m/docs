...
Accepts either a funding `transfer_id` or a withdrawal's `withdrawal_id` — checks funding transfers first, falls back to withdrawals. The response's `type` field (`"funding"` or `"withdrawal"`) tells you which one matched. See [Get Transfer Status](/api-reference/issuer/transfer-status) for the full response shape and status enum.
