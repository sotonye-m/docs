...
<Tip>
  ...
  When KYC transitions to `approved`, Yativo fires two background operations — both can fail silently. If they do, card creation will block with a specific error. Use these fallbacks:

  **1. Terms acceptance** — auto-triggered at KYC approval. If card creation returns `LEGAL_ACCEPTANCE_REQUIRED`, manually accept terms using the customer's `yativo_card_id`:
  ```bash
  POST /v1/yativo-card/{yativoCardId}/terms
  ```
  This endpoint works for both own-account and customer cards — just supply the customer's `yativo_card_id` as the path parameter. It is idempotent.

  **2. Safe (card wallet) deployment** — auto-triggered at KYC approval. If card creation returns `SAFE_NOT_DEPLOYED`, manually trigger deployment:
  ```bash
  POST /v1/yativo-card/customers/{yativoCardId}/safe/deploy
  ```
  A `200` with `"already_deployed": true` means the Safe is ready. A `202` means deployment was accepted and will complete within ~60 seconds.
</Tip>
...
| **Accept terms (fallback)** | `POST` | `/v1/yativo-card/{yativoCardId}/terms` |
