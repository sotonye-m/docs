...
| `tx_hash` | string \| null | On-chain transaction hash (when available). |
...
<Note>
  Balance fields are provided as both a decimal float (`ledger_balance`) and an integer in minor units (`ledger_balance_minor` = cents). For example, `45.23 EUR` → `ledger_balance: 45.23`, `ledger_balance_minor: 4523`. `pending_balance` / `pending_balance_minor` may be `null` when there are no pending authorizations.
</Note>
...
| `transaction_id` | string \| null | Network transaction ID — may be `null` for some transaction types |
...
<Note>
  `transaction.authorized` can fire twice for the same card transaction — once when the authorization is created and again when it is cleared. Both events carry the same `transaction_id`. Use `data.transaction_id` (not `event.id`) as the key for transaction-level deduplication in your handler. `transaction_id` may be `null` for some transaction types — in that case fall back to `event.id`.
</Note>
