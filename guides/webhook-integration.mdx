...
<Note>
  Card issuer program events (`transaction.authorized`, `transaction.settled`, `card.created`, `customer.funded`, etc.) are delivered through a separate webhook system. See [Webhook Events](/api-reference/issuer/webhooks) for the full reference and subscription setup at `POST /v1/yativo-card/webhooks`.
</Note>
