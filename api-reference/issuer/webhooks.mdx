...
Subscribe and manage your webhook endpoints via the Card Issuer Webhooks API.
...
<Warning>
  The `secret` is returned only at creation time. Store it immediately — if you lose it, rotate it via `POST /v1/yativo-card/webhooks/:webhookId/rotate-secret`.
</Warning>
...
```json
{
  ...
  "data": {
    "transfer_id": "bridge_1745128800_9fc12ab3e4d5",
